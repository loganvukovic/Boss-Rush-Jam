...
public class BossActions : MonoBehaviour
{
    ...
    public void ChooseSpot(int spot)
    {
        if (spot != prevSpot)
        {
            prevSpot = spot;
            hitCounter = 0;
            Quaternion angle = CalcNewAngle(spot);
            StartCoroutine(UpdatePosition(spot, angle));
    ...
    public IEnumerator UpdatePosition(int spot, Quaternion angle)
    {
        wontShoot = true;
        moving = true;
        ...
        while (transform.position != tempObject.transform.position)
        {
            ...
            transform.localRotation = Quaternion.Slerp(transform.localRotation, angle, 0.05f);
